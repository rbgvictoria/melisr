...
class Dna_sequence_model extends CI_Model {
    ...
    public function getMarkerDetails() {
        $sql = "SELECT d.targetMarker, if(pi.PickListItemID IS NOT NULL, 1, 0) AS isInPickList, count(*) AS cntSequences
            ...
            GROUP BY d.TargetMarker, pi.PickListItemID
            UNION
            SELECT `Value`, 1, NULL
    ...
    public function getProjects() {
        $sql = "SELECT p.ProjectID, p.ProjectName, c.CollectionName,
                count(d.DnaSequenceID) AS cntSequences
            FROM project p
            ...
            JOIN collection c ON p.CollectionMemberID=c.UserGroupScopeID
            GROUP BY p.ProjectID, c.UserGroupScopeID";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
