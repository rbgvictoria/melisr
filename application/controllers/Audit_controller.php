...
class Audit_controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('Audit_model', 'auditmodel');
    ...
    public function index($limit=FALSE, $page=FALSE) {
        $this->getTableList();
        
        if (!$limit || $this->input->post('submit')) {
            $unset = [
                'startdate',
                'enddate',
                'table',
    ...
    private function getTableList() {
        $doc = new DOMDocument();
        $doc->load(APPPATH . 'xml/specify_tableid_listing.xml');
