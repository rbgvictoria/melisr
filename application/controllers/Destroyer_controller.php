...
class Destroyer_controller extends CI_Controller {
    var $data;
    
    public function __construct() {
        parent::__construct();
        $this->load->model('Destroyer_model', 'destroyermodel');
    ...
    public function index() {
        if ($this->input->post('submit') && $this->input->post('agent') && isset($_FILES['uploadedfile']['tmp_name'])) {
                    ...
                    if ($colobjids) {
                        foreach ($colobjids as $colobj) {
                            ...
                            if ($line[1]) {
                                ...
                                if (!$preparation) {
                                    $errors[] = "Preparation for $line[0] is not in pick list.";
                                    $preparation = NULL;
                                }
                            }
                            else
                            ...
                            if ($line[2]) {
                                ...
                                if (!$eventtype) {
                                    $errors[] = "Event type for $line[0] is not in pick list.";
                                    $eventtype = NULL;
                                }
                            }
                            ...
                            if ($line[7]) {
                                ...
                                if (!$cause) {
                                    $errors[] = "Cause for $line[0] is not in pick list.";
                                    $cause = NULL;
                                }
                            }
                            ...
                            if ($line[8]) {
                                ...
                                if (!$cause) {
                                    $errors[] = "Severity for $line[0] is not in pick list.";
                                    $cause = NULL;
                                }
                            }
                            ...
                            if (isset($line[3]) && $line[3]) {
                                ...
                                if (!$researcherID) {
                                    $errors[] = 'Couldn&apos;t find agent for Researcher in ' . trim($line[0]) . '.';
                                    $researcherID = NULL;
                                }
                            }
                            ...
                            if (isset($line[10]) && $line[10]) {
                                ...
                                if (!$assessedByID) {
                                    $errors[] = 'Couldn&apos;t find agent for Assessed by in ' . trim($line[0]) . '.';
                                    $assessedByID = NULL;
                                }
                            }
                            ...
                            if (isset($line[4]) && $line[4]) {
                                ...
                                if (!$tdate) {
                                    $errors[] = 'Invalid date for Sample date in ' . trim($line[0]) . '.';
                                }
                                else {
                            ...
                            if (isset($line[9]) && $line[9]) {
                                ...
                                if (!$tdate) {
                                    $errors[] = 'Invalid date for Date noticed in ' . trim($line[0]) . '.';
                                }
                                else {
                                    $datenoticed = $tdate[0];
                            ...
                            if (isset($line[11]) && $line[11]) {
                                ...
                                if (!$tdate) {
                                    $errors[] = 'Invalid date for Date assessed in ' . trim($line[0]) . '.';
                                }
                                else {
            ...
            if ($recordids) {
                ...
                $success[] = count($recordids) . ' Conserv. Descriptions and Events have been uploaded.';
                $success[] = "Record set &apos;Destroyer upload $date&apos; has been created";
            }
